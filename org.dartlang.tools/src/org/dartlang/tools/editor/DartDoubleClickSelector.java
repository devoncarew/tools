...
/**
 * Dart double click strategy.
 */
public class DartDoubleClickSelector implements ITextDoubleClickStrategy {
  ...
  public void doubleClicked(ITextViewer text) {
    selectedPos = text.getSelectedRange().x;
   ...
   * <code>false</code> otherwise.
   * 
   * @return <code>true</code> if a word can be found at the current selection location,
   *         <code>false</code> otherwise
   */
   ...
   */
  protected int searchForClosingBracket(int startPosition, char openBracket, char closeBracket,
   ...
   * @throws BadLocationException in case <code>startPosition</code> is invalid in the document
   */
  protected int searchForOpenBracket(int startPosition, char openBracket, char closeBracket,
      IDocument document) throws BadLocationException {
    int stack = 1;
  ...
  protected boolean selectBracketBlock() {
    if (matchBracketsAt()) {
      ...
      if (startPos == endPos) {
        text.setSelectedRange(startPos, 0);
      } else {
        text.setSelectedRange(startPos + 1, endPos - startPos - 1);
      }
